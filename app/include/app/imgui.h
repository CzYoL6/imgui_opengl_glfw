...
// IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is used for the default src files (imgui_impl_xxx.h)
...
#ifndef ImTextureID
typedef void* ImTextureID;          // Default: store a pointer or an integer fitting in a pointer (most renderer src are ok with that)
#endif
...
#ifndef ImDrawIdx
typedef unsigned short ImDrawIdx;   // Default: 16-bit (for maximum compatibility with renderer src)
#endif
...
namespace ImGui
{
    ...
    IMGUI_API bool          IsAnyMouseDown();                                                   // [WILL OBSOLETE] is any mouse button held? This was designed for src, but prefer having backend maintain a mask of held mouse buttons, because upcoming input queue system will make this invalid.
...
enum ImGuiKey_
{
    ...
    //   src tend to interfere and break that equivalence. The safer decision is to relay that ambiguity down to the end-user...
...
struct ImGuiIO
{
    ...
    float       MouseWheelH;                        // Mouse wheel Horizontal. Most users don't have a mouse with an horizontal wheel, may not be filled by all src.
...
struct ImFontAtlas
{
    ...
    // - If you render colored output, set 'atlas->TexPixelsUseColors = true' as this may help some src decide of prefered texture format.
...
// Flags stored in ImGuiViewport::Flags, giving indications to the platform src.
